...
RUN echo @testing http://nl.alpinelinux.org/alpine/edge/testing >> /etc/apk/repositories && \
    ...
    curl -o /usr/local/bin/composer https://getcomposer.org/composer.phar && chmod a+x /usr/local/bin/composer && \
    pip3 install -U pip && \
...
ADD deploy/conf/supervisord.conf /etc/supervisord.conf

# Copy our nginx config
RUN rm -Rf /etc/nginx/nginx.conf
ADD deploy/conf/nginx.conf /etc/nginx/nginx.conf
...
rm -Rf /var/www/* && \
mkdir /var/www/html/
ADD deploy/conf/nginx-site.conf /etc/nginx/sites-available/default.conf
ADD deploy/conf/nginx-site-ssl.conf /etc/nginx/sites-available/default-ssl.conf
...
ADD deploy/scripts/pull /usr/bin/pull
ADD deploy/scripts/push /usr/bin/push
ADD deploy/scripts/letsencrypt-setup /usr/bin/letsencrypt-setup
ADD deploy/scripts/letsencrypt-renew /usr/bin/letsencrypt-renew
...
COPY routes routes/
RUN mkdir -p storage/framework/cache && mkdir -p storage/framework/sessions && mkdir -p storage/framework/testing && mkdir -p storage/framework/views && \
    chmod 777 -Rf storage/framework && \
    chmod 777 -Rf bootstrap
...
ADD deploy/errors/ /var/www/errors
EXPOSE 443 80
